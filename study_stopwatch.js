(() => {
  ...
  panel.innerHTML = `
    ...
    <button id="studyTimerApply">記録欄に反映</button>`;
  ...
  function renderDetailedStudyChart() {
    ...
    const max = Math.max(...items.map(item=>item.value),30), sum = items.reduce((value,item)=>value+item.value,0);
    ...
    chart.innerHTML = items.map(item => '<div class="bar-wrap"><div class="stack-bar" style="height:'+Math.max(3,item.value/max*92)+'px" title="'+minText(item.value)+'">'+item.parts.map(part=>'<span class="stack-segment" style="height:'+(part.value/item.value*100)+'%;background:'+part.color+'"></span>').join('')+'</div><span class="bar-label">'+item.label+'</span><span class="bar-detail">'+item.detail+'</span>'+(detailedChartRange==='week'?'<span class="bar-total">'+Math.floor(item.value/60)+'時間'+(item.value%60)+'分</span>':'')+'</div>').join('');
  ...
  document.getElementById('studyTimerApply').textContent = '計測分を保存';
