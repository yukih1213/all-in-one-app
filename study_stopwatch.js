(() => {
  ...
  function renderDetailedStudyChart() {
    ...
    chart.innerHTML = items.map(item => '<div class="bar-wrap"><div class="bar-plot"><div class="stack-bar '+(item.value?'has-value':'')+'" style="height:'+(item.value?Math.max(5,item.value/max*100):2)+'%" title="'+minText(item.value)+'">'+item.parts.map(part=>'<span class="stack-segment" style="height:'+(part.value/item.value*100)+'%;background:'+part.color+'"></span>').join('')+(item.value?'<span class="stack-value">'+(item.value<60?item.value+'分':Math.floor(item.value/60)+'h'+String(item.value%60).padStart(2,'0'))+'</span>':'')+'</div></div><div class="bar-meta"><span class="bar-label">'+item.label+'</span><span class="bar-detail">'+item.detail+'</span>'+(detailedChartRange==='week'?'<span class="bar-total">'+Math.floor(item.value/60)+'時間'+(item.value%60)+'分</span>':'')+'</div></div>').join('');
